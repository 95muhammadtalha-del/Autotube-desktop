...
class GenerateRequest(BaseModel):
    ...
    video_speed: float = Field(
        ...
        description="Playback speed multiplier (e.g. 1.2 for 20% faster).",
    )
    flip_horizontal: bool = Field(
        default=False,
        description="Flip the video horizontally to bypass simple fingerprinting.",
    )
    auto_face_track: bool = Field(
        default=False,
        description="Automatically track faces and generate panning keyframes.",
    )
    blur_background: bool = Field(
        default=False,
        description="Fit video to width and fill the background with a blurred version instead of cropping.",
    )
    ...
    clip_start: Optional[float] = Field(default=None, description='Override auto-selector and start at this second')
    ...
    video_volume: Optional[float] = Field(
        default=100.0, ge=0, le=200,
        description="Original video audio loudness (0-200). 100 is original.",
    )
    ...
    edge_crop: float = Field(
        default=0.0, ge=0.0, le=50.0, description="Percentage of edges to crop (0-50)."
    )
    zoom_factor: float = Field(
        default=1.0, ge=1.0, le=3.0, description="Static zoom scale multiplier."
    )
    audio_pitch: float = Field(
        default=1.0, ge=0.5, le=2.0, description="Audio pitch shift factor."
    )
