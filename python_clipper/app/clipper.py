...
class ClipOptions:
    ...
    video_volume: float = 100.0        # 0-200, original audio volume
    ...
    video_speed: float = 1.0           # playback speed multiplier
    flip_horizontal: bool = False      # horizontally mirror the video
    blur_background: bool = False      # use a blurred background padding instead of crop
    edge_crop: float = 0.0             # edge cropping percentage (0-50)
    zoom_factor: float = 1.0           # static zoom multiplier (1.0 - 3.0)
    audio_pitch: float = 1.0           # audio pitch shift factor (0.5 - 2.0)
...
def _finish_stages(in_label: str, vw: int, vh: int, opts: ClipOptions, work_dir: Path) -> list[str]:
    ...
    if getattr(opts, "flip_horizontal", False):
        cine_stages.append(f"[{cap_in}]hflip[cap_in_f]")
        cap_in = "cap_in_f"
...
def _crop_filter(width: int, height: int, reframe: Optional[list[dict]], base_zoom: float = 1.0) -> str:
    ...
    if xf is None and yf is None:
        if base_zoom > 1.001:
            return f"scale=w='iw*{base_zoom}':h='ih*{base_zoom}',crop={width}:{height}"
        return f"crop={width}:{height}"
    xf = xf or "0.5"
    yf = yf or "0.5"

    if zoom is not None or base_zoom > 1.001:
        z_expr = zoom if zoom is not None else "1.0"
        # Total zoom is the dynamic keyframe zoom * the static base zoom
        tot_z = f"({z_expr}*{base_zoom})"
        ...
        # a centred one.
        prefix = f"scale=w='iw*{tot_z}':h='ih*{tot_z}':eval=frame,"
        x_part = f"({width}*{tot_z}-{width})*({xf})"
        y_part = f"({height}*{tot_z}-{height})*({yf})"
    else:
        prefix = ""
...
def _build_crop_filter_complex(width: int, height: int, opts: ClipOptions, work_dir: Path) -> str:
    ...
    # base_zoom = static zoom * edge crop zoom
    base_zoom = getattr(opts, "zoom_factor", 1.0) * (1.0 + getattr(opts, "edge_crop", 0.0) / 100.0)
    
    if getattr(opts, "blur_background", False):
        bg = f"[0:v]scale={width}:{height}:force_original_aspect_ratio=increase,crop={width}:{height},boxblur=20:5,colorchannelmixer=rr=.4:gg=.4:bb=.4[bg]"
        fg = f"[0:v]scale={width}:{height}:force_original_aspect_ratio=decrease[fg]"
        ov = f"[bg][fg]overlay=(main_w-overlay_w)/2:(main_h-overlay_h)/2[v0]"
        stages = [bg, fg, ov]
    else:
        stages = [
            f"[0:v]scale={width}:{height}:force_original_aspect_ratio=increase,"
            f"{_crop_filter(width, height, opts.reframe, base_zoom)}[v0]"
        ]
...
def _build_square_filter_complex(opts: ClipOptions, work_dir: Path) -> str:
    ...
    stages = [
        ...
        f"{_crop_filter(s, s, opts.reframe, base_zoom)}[fgsq]",
    ]
    ...
    if getattr(opts, "flip_horizontal", False):
        stages.append(f"[{sq_cine}]hflip[sq_cine_f]")
        sq_cine = "sq_cine_f"
...
def _music_audio_graph(voice_in: str, mus_idx: int, vid_volume: float, mus_volume: float, duck: float = 70.0) -> str:
    ...
    ``mus_volume`` (0-100) sets the music's resting loudness; ``duck`` (0-100) sets how
    ...
    """
    v_base = (vid_volume if vid_volume is not None else 100.0) / 100.0
    m_base = max(0.0, min(1.0, (mus_volume if mus_volume is not None else 35.0) / 100.0 * 0.7))
    d = max(0.0, min(1.0, (duck if duck is not None else 70.0) / 100.0))
    ratio = 1.0 + d * 19.0  # 0 → 1 (no ducking), 100 → 20 (hard ducking)
    return (
        f"[{voice_in}]volume={v_base:.3f},asplit=2[__v1][__v2];"
        f"[{mus_idx}:a]volume={m_base:.3f},aresample=async=1[__m];"
...
def generate_clip(source_mp4: Path, start: float, end: float, opts: ClipOptions, progress_callback=None) -> Path:
    ...
    # Pitch shift
    audio_pitch = getattr(opts, "audio_pitch", 1.0)
    voice_in_label = "0:a"
    if abs(audio_pitch - 1.0) > 0.01:
        fc = fc + f";[0:a]rubberband=pitch={audio_pitch}[pitch_a]"
        voice_in_label = "pitch_a"
    ...
    if has_music:
        ...
        fc = fc + ";" + _music_audio_graph(voice_in_label, music_idx, opts.video_volume, opts.music_volume, opts.music_duck)
        audio_out_label = "[aout]"
    else:
        if opts.video_volume != 100.0:
            v_base = opts.video_volume / 100.0
            fc = fc + f";[{voice_in_label}]volume={v_base:.3f}[aout]"
            audio_out_label = "[aout]"
        else:
            audio_out_label = f"[{voice_in_label}]" if voice_in_label != "0:a" else "0:a"
    ...
    if opts.video_speed != 1.0:
        ...
        # if audio_out_label is "0:a", we add the ? so it doesn't fail on silent videos
            ...
            if progress_callback and "time=" in line:
                ...
                if match:
                    ...
                    if duration > 0:
                        ...
                        progress_callback(frac, f"Rendering video... {int(frac * 100)}%")

        proc.wait()
        if proc.returncode != 0:
            raise ClipGenerationError(f"Failed to run ffmpeg, exit code {proc.returncode}")
    except FileNotFoundError as exc:
        raise ClipGenerationError(
