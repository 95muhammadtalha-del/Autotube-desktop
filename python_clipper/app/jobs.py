...
from . import captions, downloader, facetrack, history, music, pretranscribe, reframe, selector, transcriber, uploads
...
def _run_pipeline(job: Job) -> None:
    ...
    try:
        ...
        if getattr(req, "clip_start", None) is not None:
        ...
        for index, win in enumerate(windows):
            ...
            if getattr(req, "auto_face_track", False):
                job.set_stage(
                    "rendering",
                    index / total,
                    f"Clip {index + 1}/{total}: Scanning for faces...",
                )
                try:
                    reframe_keyframes = facetrack.generate_keyframes(source_mp4, start, end)
                except Exception as e:
                    logger.warning("[%s] facetrack failed: %s", job.id, e)
            ...
            opts = ClipOptions(
                ...
                video_volume=req.video_volume if req.video_volume is not None else 100.0,
            ...
            def on_render_progress(frac: float, msg: str) -> None:
                job.set_stage("rendering", frac, f"Clip {index + 1}/{total}: {msg}")

            try:
                out = generate_clip(source_mp4, start, end, opts, progress_callback=on_render_progress)
                logger.info("[%s] generated clip %d: %s", job.id, index, out.name)
            except Exception as exc:
                raise ClipGenerationError(f"Clip {index+1} failed: {exc}") from exc
            ...
            reframe.save_recipe(
                ...
                opts_kwargs={
                    ...
                    "video_speed": opts.video_speed,
                    "flip_horizontal": opts.flip_horizontal,
                    "blur_background": opts.blur_background,
                },
            )
